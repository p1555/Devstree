import { students as initialStudents } from "./info.js";
...
function buildStudentObject() {
  ...
  if (idVal) {
    // Edit Mode
    const existing = getStudents().find((s) => s.id === Number(idVal));
    subjects = existing.academics.subjects.map((s, i) => {
      const input = document.querySelector(`#subjectInputs input[data-index="${i}"]`);
      return {
        ...s,score: Number(input?.value) || 0,
      };
    });
    totalAcademicDays = Number(document.getElementById("totalDays").value);
    presentDays = Number(document.getElementById("presentDays").value);
  } else {
    // New student
    subjects = initialStudents[0].academics.subjects.map((subj) => ({
  ...
  const academics = {
    total_subject_scores: subjects.length * subjects[0].total,
    subjects,
  };
  ...
  return {
    id: idVal? Number(idVal) : getStudents().length ? Math.max(...getStudents().map((s) => s.id)) + 1: 1,
    personal,
    academics,
    attendance,
  };
...
function openForm(student = null, isReadOnly = false) {
  ...
  document.getElementById("edit-only-fields").style.display = isEdit ? "block" : "none";
  ...
  if (student) {
    ...
    student.academics.subjects.forEach((subj, i) => {
      const input = document.createElement("input");
      input.type = "number";
      input.min = 0;
      input.max = subj.total;
      input.value = subj.score;
      input.dataset.index = i;
      input.placeholder = `Score for Subject ${i + 1}`;
      subjectContainer.appendChild(input);
      subjectContainer.appendChild(document.createElement("br"));
    });

    // Attendance
    document.getElementById("totalDays").value = student.attendance.totalAcademicDays || 0;
    document.getElementById("presentDays").value = student.attendance.presentDays || 0;
  }
...
searchInput.addEventListener("input", () => {
  ...
  renderStudents(
    getStudents().filter( (s) =>
        s.personal.name.toLowerCase().includes(q) || s.id.toString().includes(q) )
  );
});

gradeFilter.addEventListener("change", () => {
  const min = Number(gradeFilter.value);
  renderStudents(min ? getStudents().filter( (s) => calculateAvgGrade(s.academics.subjects) > min ) : getStudents());
});

attendanceFilter.addEventListener("change", () => {
  const max = Number(attendanceFilter.value);
  renderStudents(max? getStudents().filter((s) => calculateAttendance(s.attendance) < max): getStudents()  );
});
renderStudents();